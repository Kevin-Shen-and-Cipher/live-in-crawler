...
class Apartment591Carwler(ApartmentCrawler):
    ...
    def get_district(self, district):
        ...
        return super().get_district(district)
        ...
        try:
            for i in self.region:
                ...
                while data_count < self.data_limit:
                    region_url = "https://rent.591.com.tw/?region=%s&firstRow=%s" % (
                        str(i),
                        str(page_row),
                    )
                    ...
                    rent_list = page_element.find_all(
                    ...
                    for rent_item in rent_list:
                        ...
                        web_data = self.get_web_data(
                            rent_item.find("a", href=True)["href"]
                        )
                        if web_data != None:
                            result = self.post_data(web_data)
                            if result.status_code != 201:
                                self.post_error(
                                    result.text, web_data["url"].split("detail-")[1]
                                )
                            ...
                            print(
                                "data error %s" % rent_item.find("a", href=True)["href"]
                            )
                    page_row += 30
    ...
    def get_web_data(self, rent_url: str):
        try:
            ...
            house_info = house_content.find_element(
                By.XPATH, "//section[@class='house-info']"
            )
            data["name"] = house_info.find_element(
                By.XPATH, "//div[@class='house-title']//h1"
            ).text
            data["price"] = int(
                house_info.find_element(
                    By.XPATH, "//div[@class='house-price']//span[@class='price']//b"
                ).text.replace(",", "")
            )
            data["address"] = house_content.find_element(By.CLASS_NAME, "load-map").text
            data["district"] = self.get_district(data["address"])
            data["coordinate"] = self.DMS_to_DD(
                house_content.find_element(By.CSS_SELECTOR, "div.lat-lng")
                .get_attribute("innerHTML")
                .replace("'", "'")
            )

            # house pattern section
            house_pattern = house_info.find_element(
                By.CLASS_NAME, "house-pattern"
            ).find_elements(By.TAG_NAME, "span")
            data["rent_type"] = self.get_rent_type(house_pattern[0].text)
            data["apartment_type"] = self.get_aparment_type(
                house_pattern[len(house_pattern) - 1].text
            )
            data["room_type"] = self.get_room_type(house_pattern)

            # get restrict section
            try:
                data["restrict"] = self.get_restrict(
                    house_content.find_element(By.CLASS_NAME, "service-rule")
            ...
            # get device section
            try:
                data["device"] = self.get_device(
                    house_content.find_element(
                        By.CLASS_NAME, "service-list-box"
                    ).find_elements(By.CLASS_NAME, "service-list-item")
                )
            except Exception as e:
            ...
            data["surrounding_facility"] = self.get_facility_type(
                self.browser.find_element(By.ID, "surround-map").find_elements(
